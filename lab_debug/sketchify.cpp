...
RGBAPixel* myFavoriteColor(int blueval) {
    /*
     * the code commented below is very bad.
     * it allocates a RGBAPixel as a struct on the stack.
     * it then returns a reference to the pixel
     * since it returns, the stack frame, including the pixel, is freed
     * so any reference to the pixel outside of the code is a segfault
     * this is why lifetimes are so important. a reference should NEVER 
